...
class SaveTagsCommand(Command):
    name = "elk-save-tags"
    description = "Save packaging tags for the current system to a {name}.tags file"
    ...
    def handle(self) -> int:
        ...
        output_path = Path(f"{plat_name}.tags")
        tag_list = list(sys_tags())

        with open(output_path, "w") as f:
            f.write(f"# Tags for {platform_mod.system()} {platform_mod.machine()}\n")
            f.write(f"# Python {platform_mod.python_version()}\n")
            f.write(f"# Generated by: poetry elk-save-tags {plat_name}\n")
            f.write(f"#\n")
            f.write(f"# To regenerate, run on the target machine:\n")
            f.write(f"#   poetry elk-save-tags {plat_name}\n")
            f.write(f"# Or without elk installed:\n")
            f.write(
                f'#   python3 -c "from packaging.tags import sys_tags; '
                f'[print(t) for t in sys_tags()]"'
                f" > {output_path}\n"
            )
            for tag in tag_list:
                f.write(f"{tag}\n")
